    ...
    <div className="app-shell">
      <div className="page">
        <div style={{ padding: '20px 16px 0' }}>
          ...
          {/* Cabeçalho do perfil — card gradiente */}
          <div style={{
            background: 'linear-gradient(135deg, #33cccc 0%, #1aabab 100%)',
            borderRadius: 20, padding: '18px 18px 20px', marginBottom: 20, color: 'white',
          }}>
                ...
                {isLoggedIn && (
                  <button
                    ...
                    style={{
                      ...
                      background: 'white', border: '2px solid #1aabab',
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      cursor: 'pointer',
                    }}
                    title="Escolher avatar"
                  >
                    <svg width="9" height="9" viewBox="0 0 12 12" fill="none" stroke="#1aabab" strokeWidth="2.2" strokeLinecap="round">
              ...
              {editMode ? (
                <div style={{ flex: 1 }}>
                  <input
                    ...
                    style={{ width: '100%', background: 'rgba(255,255,255,0.18)', border: '1.5px solid rgba(255,255,255,0.5)', borderRadius: 10, padding: '7px 12px', fontFamily: 'var(--font)', fontSize: 14, color: 'white', marginBottom: 6, outline: 'none' }}
                  />
                  <div style={{ position: 'relative' }}>
                    <span style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', color: 'rgba(255,255,255,0.7)', fontSize: 13 }}>@</span>
                    <input
                      value={editUsername}
                      onChange={e => setEditUsername(e.target.value.toLowerCase().replace(/[^a-z0-9_.]/g, ''))}
                      placeholder="usuario"
                      style={{ width: '100%', background: 'rgba(255,255,255,0.18)', border: '1.5px solid rgba(255,255,255,0.35)', borderRadius: 10, padding: '7px 12px 7px 24px', fontFamily: 'var(--font)', fontSize: 14, color: 'white', outline: 'none' }}
                    />
                  </div>
                  <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                    <button onClick={saveProfile} disabled={saving} style={{ flex: 1, padding: '9px 0', fontSize: 13, borderRadius: 50, border: 'none', background: 'white', color: '#1aabab', fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font)' }}>
                      {saving ? 'Salvando...' : 'Salvar'}
                    </button>
                    <button onClick={() => setEditMode(false)} style={{ flex: 1, padding: '9px 0', fontSize: 13, borderRadius: 50, border: '1.5px solid rgba(255,255,255,0.5)', background: 'transparent', color: 'white', fontWeight: 600, cursor: 'pointer', fontFamily: 'var(--font)' }}>
                      Cancelar
                    </button>
                  </div>
                </div>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 700, fontSize: 17, color: 'white', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {profile.display_name || (isLoggedIn ? 'Completar perfil' : 'Visitante')}
                  </div>
                  <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.8)', marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {profile.username
                      ? `@${profile.username}${isLoggedIn ? ' · Plano Grátis' : ''}`
                      : isLoggedIn ? 'Toque em Editar para adicionar @' : ''}
                  </div>
                </div>
              )}
              ...
              {isLoggedIn && !editMode && (
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 50, padding: '7px 16px', color: 'white', fontSize: 13, fontWeight: 600,
                ...
                <>
                  <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginTop: 20 }}>
                    <div style={{ display: 'flex', gap: 28 }}>
                      <div>
                        <div style={{ fontSize: 24, fontWeight: 800, lineHeight: 1.1 }}>{checkins}</div>
                        <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.8)', marginTop: 2 }}>Check-ins</div>
                      </div>
                      <div>
                        <div style={{ fontSize: 24, fontWeight: 800, lineHeight: 1.1 }}>{avaliacoes}</div>
                        <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.8)', marginTop: 2 }}>Avaliações</div>
                      </div>
                    </div>
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: '#5b21b6', marginBottom: 2 }}>Faz parte da rede de apoio às famílias?</div>
              <div style={{ fontSize: 12, color: '#7c3aed', lineHeight: 1.4 }}>Doula, consultora, pediatra, psicóloga e muito mais — alcance famílias que buscam apoio no pré e pós-parto, na sua cidade ou em viagem.</div>
            </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'flex-end' }}>
          <div style={{ background: 'var(--bg-card)', borderTopLeftRadius: 24, borderTopRightRadius: 24, width: '100%', maxHeight: '85vh', overflowY: 'auto', padding: '20px 20px 48px' }}>
            ...
            <div style={{ fontSize: 14, color: 'var(--text-secondary)', lineHeight: 1.7, marginBottom: 20 }}>
              Porque sair ou viajar com crianças pequenas não precisa ser estressante. Com as informações certas — e com a ajuda uns dos outros — cada parada pode ser um bom momento. Ajude-nos a construir uma rede de apoio cada vez mais participativa e "ninhos" cada vez mais acolhedores.
            </div>
